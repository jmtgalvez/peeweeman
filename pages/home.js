...
export default function Home() {
    ...
    useEffect(() => {
        const userData = user ? null : localStorage.getItem('peeweeman-user');
        if (!user && !userData)
            router.push('/login');
        else {
            ...
            fetchAccounts(credentials)
                .then( res => {
                    ...
                    let fetchedAccounts = 
                        credentials.accounts === null || credentials.accounts === undefined
                            ? { accounts: [] }
                            : JSON.parse((CryptoAES.decrypt(credentials.accounts, `${credentials.username}@${credentials.password}`)).toString(CryptoENC))
    ...
    useEffect(() => {
        if (accounts && accounts.length > 0) saveAccount();
    }, [accounts])
